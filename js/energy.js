addLayer("en", {
    ...
    startData() { return {
        ...
        gen9amt: new Decimal(0),
        gen10amt: new Decimal(0),
        gen11amt: new Decimal(0),
        gen12amt: new Decimal(0),
        ...
        gen9gain: new Decimal(0),
        gen10gain: new Decimal(0),
        gen11gain: new Decimal(0),
        gen12gain: new Decimal(0),
        ...
        gen9multi: new Decimal(1),
        gen10multi: new Decimal(1),
    ...
    exponent() {
        if (hasAchievement("a", 75)) return 0.35
        return 0.4
            ...
            content: [
                ...
                ["display-text",
                    function(){
                        ...
                        if (hasUpgrade("pa", 22)) { a = a + ` and molecules by
                        <h2><span style="color: #A3D5FF; font-family: Lucida Console, Courier New, monospace">
                            ${notationChooser(player.en.power.add(1).pow(player.en.powerexpomolecule))}</span></h2>` }
                ...
                ["display-text", function() {
                    if (player.en.gen8amt.gte(1)) return "You have "+ notationChooser(player.en.gen8amt) +" Generator 8. (+" + notationChooser(player.en.gen8gain) + "/s)"
                }], 
                ["display-text", function() {
                    if (player.en.gen9amt.gte(1)) return "You have "+ notationChooser(player.en.gen9amt) +" Generator 9." 
                }], 
                "blank",
                "blank",
                "prestige-button",
    ...
    upgrades: {
        ...
        84: {
            ...
            effect() {
                ...
                if (hasUpgrade("mo", 43)) powerful = 1
                sdsc = ""
        ...
        91: {
            title: "Transcending Whee",
            description: "Energy upgrade 'wheeeeeeeeeeeee' now has 33 more 'e's (Current: 52). 'e's boost Energy by 1.2^eamt (prev: +x1/eamt), Matter by 1.05^eamt (prev:+x0.25/eamt), Molecules by 1.01^eamt",
            cost() {
                let cost = new Decimal("4e1642")
                return cost
            },
            unlocked() { return (hasMilestone("cf", 5) && hasUpgrade("en", 85)) }, 
        },
        92: {
            title: "Major Trade off",
            description: "Energy prestige exponent is weaker (0.4 -> 0.35), but 'Atomic Duplication', 'Electric', and 'Atomic Amplifier' is stronger.",
            cost: new Decimal("1e1671"),
            unlocked() { return (hasMilestone("cf", 5) && hasUpgrade("en", 91)) }, 
        },
        93: {
            title: "Generator 9 OP",
            description: "This upgrade has 4 effects. 1: Generator 9 gives 1 extra Generator 1-8. 2: Generator 9's scaling is weaker (xe90 -> xe69). 3: Every 8 Generator 3 now gives 1 extra Generator 1. 4: ^1.015 Power. Permanently Keep 'Major Trade off'.",
            cost: new Decimal("1e2112"),
            unlocked() { return (hasMilestone("cf", 5) && hasUpgrade("en", 92)) }, 
        },
        94: {
            title: "Tickspeed+",
            description: "Autobuy 'Tickspeed' buyable, and the effect is stronger (1.11 -> 1.125). Also, 'Generator 1''s 'Generator²' effect is stronger.",
            cost: new Decimal("3e2363"),
            unlocked() { return (hasMilestone("cf", 5) && hasUpgrade("en", 93)) }, 
        },
        95: {
            title: "Super Atom Factory",
            description: "x1e25 Atoms.",
            cost: new Decimal("2.6e2636"),
            unlocked() { return (hasMilestone("cf", 5) && hasUpgrade("en", 94)) }, 
        },
    },
    buyables: {
        11: {
            ...
            display() {
                ...
                if (this.extra().gte(1)) {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " + " + notationChooser(this.extra()) + " Generator 1."
                } else {
            ...
            extra(){
                ...
                if (hasUpgrade("en", 93)) extra = extra.plus(getBuyableAmount(this.layer, 21).div(8).floor())
                return extra
            },
            effect() {
                ...
                if (hasUpgrade("en", 94)) gensqboost = new Decimal(1.0625)
                if (hasUpgrade("en", 33)) {
        ...
        12: {
            ...
            display() {
                let dis = "Cost: " + notationChooser(tmp[this.layer].buyables[this.id].cost) + " Energy. <br>"
                if (this.extra().gte(1)) {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " + " + notationChooser(this.extra()) + " Generator 2."
                } else {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " Generator 2."
                }
            ...
            extra(){
                let extra = new Decimal(0)
                if (hasUpgrade("mo", 41)) extra = extra.plus(getBuyableAmount(this.layer, 22).div(3.5).floor())
                if (hasUpgrade("en", 93)) extra = extra.plus(getBuyableAmount(this.layer, 61))
                return extra
            },
            effect() {
        ...
        21: {
            ...
            display() {
                let dis = "Cost: " + notationChooser(tmp[this.layer].buyables[this.id].cost) + " Energy. <br>"
                if (this.extra().gte(1)) {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " + " + notationChooser(this.extra()) + " Generator 3."
                } else {
        ...
        22: {
            ...
            display() {
                let dis = "Cost: " + notationChooser(tmp[this.layer].buyables[this.id].cost) + " Energy. <br>"
                if (this.extra().gte(1)) {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " + " + notationChooser(this.extra()) + " Generator 4."
                } else {
        ...
        32: {
            ...
            display() {
                let dis = "Cost: " + notationChooser(tmp[this.layer].buyables[this.id].cost) + " Energy. <br>"
                if (this.extra().gte(1)) {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " + " + notationChooser(this.extra()) + " Generator 6."
                } else {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " Generator 6."
                }
                if (hasUpgrade("en", 33)) { 
        ...
        41: {
            ...
            display() {
                let dis = "Cost: " + notationChooser(tmp[this.layer].buyables[this.id].cost) + " Energy. <br>"
                if (this.extra().gte(1)) {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " + " + notationChooser(this.extra()) + " Generator 7."
                } else {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " Generator 7."
                }
                if (hasUpgrade("en", 33)) { 
        ...
        61: {
            title: "Buy Generator 9",
            unlocked() { return getBuyableAmount("en", 42).gte(100) },
            cost(x) {
                gen9scale = "1e90"
            ...
            display() {
                let dis = "Cost: " + notationChooser(tmp[this.layer].buyables[this.id].cost) + " Energy. <br>"
                if (0 != 0) {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " + " + formatWhole(this.extra()) + " Generator 9."
                } else {
                    dis = dis + "You have " + notationChooser(getBuyableAmount(this.layer, this.id)) + " Generator 9."
                }
                dis = dis + " Generator 9 amount multiply Generator 8 generation, power and atoms gain "
                dis = dis + "by " + notationChooser(buyableEffect(this.layer, this.id)) + "."
                return dis
            },
            extra(){
    ...
    infoboxes: {
        ...
        gent2: {
            title: "Generators 2",
            body() { return "Generator 9 is the First Tier 2 generator, Tier 2 generators are highlighted in Orange and divide Atom and Energy and minus Energy, and set the Buyable amount of the last Generator to Zero. Every passing Gen 2 generator will have worse and worse nerfs but better and better buffs. To compensate, T2 Generators boost Atoms as well."},
            unlocked() {return player.en.gen8amt.gte(100)}
        }
    },
    gainMult() { // Energy
        ...
        if (hasUpgrade("mo", 23)) {
            if (hasUpgrade("en", 91)) {
		        mult = mult.times(new Decimal(1.2).pow(player.en.wheeamt))
            } else {
                if (hasUpgrade("en", 81)) mult = mult.times(player.en.wheeamt)
            }
	    } else {
		    if (hasUpgrade("en", 81)) mult = mult.times(player.en.wheeamt).div(4)
	    }
    ...
    update(diff) {
        if (hasUpgrade("en", 25) || hasMilestone("ma", 4) || hasUpgrade("ma", 21)) {
            // mults
            ...
            player.en.gen7multi = player.en.gen7multi.times(buyableEffect("mo", 41))
            ...
            if (hasAchievement("a", 73)) player.en.gen1multi = player.en.gen1multi.times(1.09)
            ...
            if (hasMilestone("ma", 14)) player.en.powerexpomatter = new Decimal(0.0125)
            if (hasUpgrade("pa", 22)) player.en.powerexpomolecule = new Decimal(0.0015)
            if (hasUpgrade("pa", 22)) player.en.powerexpoparticle = new Decimal(0.0005)
            if (hasUpgrade("mo", 44)) player.en.powerexpoparticle = new Decimal(0.001)
            if (hasUpgrade("mo", 44)) player.en.powerexpomolecule = new Decimal(0.003)
            ...
            player.en.gen9amt = getBuyableAmount("en", 61)
            
            if (getBuyableAmount("en", 61).gte(1)) {
                player.en.gen8amt = player.en.gen8amt.sub(getBuyableAmount("en", 42))
                player.en.gen8gain = player.en.gen9amt.times(player.en.gen9multi).div(25)
                player.en.gen8amt = player.en.gen8amt.add(player.en.gen8gain.times(diff))
                player.en.gen8amt = player.en.gen8amt.add(getBuyableAmount("en", 42))
            } else {
            ...
            if (getBuyableAmount("en", 32).gte(1)) {
                ...
                if (hasUpgrade("ma", 21)) {
                    player.en.gen5amt = Decimal.max(getBuyableAmount("en", 31), new Decimal(10))
                } else {
                    player.en.gen5amt = getBuyableAmount("en", 31)
                }
            }

            if (player.en.gen5amt.gte(1) || hasMilestone("ma", 4)) {
            ...
            if (getBuyableAmount("en", 22).gte(1) || hasMilestone("ma", 4) || hasUpgrade("ma", 21)) {
            ...
            if (getBuyableAmount("en", 21).gte(1) || hasMilestone("ma", 4) || hasUpgrade("ma", 21)) {
            ...
            if (getBuyableAmount("en", 12).gte(1) || hasMilestone("ma", 4) || hasUpgrade("ma", 21)) {
            ...
            if (hasMilestone("ma", 4) && player.en.gen4amt.lt(1)) {
            ...
            if (hasUpgrade("mo", 45)) gain = gain.times(upgradeEffect("mo", 45))
            ...
            if (hasUpgrade("pa", 23)) gain = gain.div(layers.pa.getBetaEff().pow(0.5))
        ...
        // ngu
            if (player.timePlayed < 60) {
                ...
                player.cf.ngu = new Decimal(player.timePlayed / 60).pow(new Decimal(player.timePlayed / 60))
            } else if (player.timePlayed < 5000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / 10).pow(new Decimal(player.timePlayed / 60)).pow(new Decimal(player.timePlayed / 1000)))
            } else if (player.timePlayed < 20000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / 10).pow(new Decimal(player.timePlayed / 60)).pow(new Decimal(player.timePlayed / 1000)).pow(new Decimal(player.timePlayed / 5000)))
            } else if (player.timePlayed < 50000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / 10).pow(new Decimal(player.timePlayed / 60)).pow(new Decimal(player.timePlayed / 1000)).pow(new Decimal(player.timePlayed / 5000)).pow(new Decimal(player.timePlayed / 10000)).pow(new Decimal(player.timePlayed / 25000)))
            } else if (player.timePlayed < 100000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (14 - Math.log10(player.timePlayed))).pow(new Decimal(player.timePlayed / 60)).pow(new Decimal(player.timePlayed / 1000)).pow(new Decimal(player.timePlayed / 5000)).pow(new Decimal(player.timePlayed / 10000)).pow(new Decimal(player.timePlayed / 25000)))
            } else if (player.timePlayed < 250000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (14 - Math.log10(player.timePlayed))).pow(new Decimal(player.timePlayed / 60)).pow(new Decimal(player.timePlayed / 1000)).pow(new Decimal(player.timePlayed / 5000)).pow(new Decimal(player.timePlayed / 10000)).pow(new Decimal(player.timePlayed / 25000)).pow(new Decimal(player.timePlayed / 50000)).pow(new Decimal(player.timePlayed / 125000)))
            } else if (player.timePlayed < 500000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (14 - Math.log10(player.timePlayed))).pow(new Decimal(player.timePlayed / (60 / Math.max(Math.log10(player.timePlayed / 175000)+1, 1))).pow(new Decimal(player.timePlayed / (700 / Math.max(Math.log10(player.timePlayed / 275000)+1, 0.8)))).pow(new Decimal(player.timePlayed / (5000 / Math.max(2*(Math.log10(player.timePlayed / 400000)+1)-1, 1))))).pow(new Decimal(player.timePlayed / 10000)).pow(new Decimal(player.timePlayed / 25000)).pow(new Decimal(player.timePlayed / 40000)).pow(new Decimal(player.timePlayed / 125000)))
            } else if (player.timePlayed < 1000000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (12 - Math.log10(player.timePlayed))).pow(new Decimal(player.timePlayed / (30 / Math.max(Math.log10(player.timePlayed / 175000)+1, 1))).pow(new Decimal(player.timePlayed / (700 / Math.max(Math.log10(player.timePlayed / 275000)+1, 0.8)))).pow(new Decimal(player.timePlayed / (4000 / Math.max(2*(Math.log10(player.timePlayed / 400000)+1)-1, 1))))).pow(new Decimal(player.timePlayed / 10000)).pow(new Decimal(player.timePlayed / 25000)).pow(new Decimal(player.timePlayed / 30000)).pow(new Decimal(player.timePlayed / 125000)).pow(new Decimal(player.timePlayed / 600000)))
            } else if (player.timePlayed < 1500000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (12 - Math.log10(player.timePlayed) - Math.log10(player.timePlayed / 100000))).pow(new Decimal(player.timePlayed / (25 / Math.max(Math.log10(player.timePlayed / 175000)+1, 1))).pow(new Decimal(player.timePlayed / (700 / Math.max(Math.log10(player.timePlayed / 275000)+1, 0.8)))).pow(new Decimal(player.timePlayed / (3000 / Math.max(2*(Math.log10(player.timePlayed / 400000)+1)-1, 1)))).pow(new Decimal(player.timePlayed / 10000)).pow(new Decimal(player.timePlayed / 25000)).pow(new Decimal(player.timePlayed / 25000)).pow(new Decimal(player.timePlayed / 125000)).pow(new Decimal(player.timePlayed / 600000))))
            } else if (player.timePlayed < 2500000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (12 - Math.log10(player.timePlayed) - Math.log10(player.timePlayed / 100000))).pow(new Decimal(player.timePlayed / (20 / Math.max(Math.log10(player.timePlayed / 175000)+1, 1))).pow(new Decimal(player.timePlayed / (700 / Math.max(Math.log10(player.timePlayed / 275000)+1, 0.8)))).pow(new Decimal(player.timePlayed / (2500 / Math.max(2*(Math.log10(player.timePlayed / 400000)+1)-1, 1)))).pow(new Decimal(player.timePlayed / 10000)).pow(new Decimal(player.timePlayed / 25000)).pow(new Decimal(player.timePlayed / 25000)).pow(new Decimal(player.timePlayed / 100000)).pow(new Decimal(player.timePlayed / 600000)).pow(new Decimal(player.timePlayed / 1.5e6))))
            } else if (player.timePlayed < 3500000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (12 - Math.log10(player.timePlayed) - Math.log10(player.timePlayed / 100000))).pow(new Decimal(player.timePlayed / (15 / Math.max(Math.log10(player.timePlayed / 175000)+1, 1))).pow(new Decimal(player.timePlayed / (700 / Math.max(Math.log10(player.timePlayed / 275000)+1, 0.8)))).pow(new Decimal(player.timePlayed / (2000 / Math.max(2*(Math.log10(player.timePlayed / 400000)+1)-1, 1)))).pow(new Decimal(player.timePlayed / 10000)).pow(new Decimal(player.timePlayed / 25000)).pow(new Decimal(player.timePlayed / 20000)).pow(new Decimal(player.timePlayed / 100000)).pow(new Decimal(player.timePlayed / 500000)).pow(new Decimal(player.timePlayed / 1.5e6)).pow(new Decimal(player.timePlayed / 2.5e6))))
            } else if (player.timePlayed < 8000000) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (12 - Math.log10(player.timePlayed) - Math.log10(player.timePlayed / 60000))).pow(new Decimal(player.timePlayed / (10 / Math.max(Math.log10(player.timePlayed / 175000)+1, 1))).pow(new Decimal(player.timePlayed / (400 / Math.max(Math.log10(player.timePlayed / 275000)+1, 0.8)))).pow(new Decimal(player.timePlayed / (1500 / Math.max(2*(Math.log10(player.timePlayed / 385000)+1)-1, 1)))).pow(new Decimal(player.timePlayed / 7500)).pow(new Decimal(player.timePlayed / 25000)).pow(new Decimal(player.timePlayed / 15000)).pow(new Decimal(player.timePlayed / 80000)).pow(new Decimal(player.timePlayed / 300000)).pow(new Decimal(player.timePlayed / 1.5e6)).pow(new Decimal(player.timePlayed / 2.5e6)).pow(new Decimal(player.timePlayed / 4e6)).pow(new Decimal(player.timePlayed / 6e6))))
            } else if (player.timePlayed < 15e6) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (11.75 - Math.log10(player.timePlayed) - Math.log10(player.timePlayed / 60000))).pow(new Decimal(player.timePlayed / (4 / Math.max(Math.log10(player.timePlayed / 110000)+1, 1))).pow(new Decimal(player.timePlayed / (400 / Math.max(Math.log10(player.timePlayed / 275000)+1, 0.8)))).pow(new Decimal(player.timePlayed / (1000 / Math.max(2*(Math.log10(player.timePlayed / 250000)+1)-1, 1)))).pow(new Decimal(player.timePlayed / 7500)).pow(new Decimal(player.timePlayed / 16000)).pow(new Decimal(player.timePlayed / 10000)).pow(new Decimal(player.timePlayed / 50000)).pow(new Decimal(player.timePlayed / 200000)).pow(new Decimal(player.timePlayed / 1.5e6)).pow(new Decimal(player.timePlayed / 2.5e6)).pow(new Decimal(player.timePlayed / 4e6)).pow(new Decimal(player.timePlayed / 6e6)).pow(new Decimal(player.timePlayed / 1e7))))
            } else if (player.timePlayed < 25e6) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (11.75 - Math.log10(player.timePlayed) - Math.log10(player.timePlayed / 60000))).pow(new Decimal(player.timePlayed / (1 / Math.max(Math.log10(player.timePlayed / 40000)+1, 1))).pow(new Decimal(player.timePlayed / (300 / Math.max(Math.log10(player.timePlayed / 250000)+1, 0.8)))).pow(new Decimal(player.timePlayed / (500 / Math.max(2*(Math.log10(player.timePlayed / 100000)+1)-1, 1)))).pow(new Decimal(player.timePlayed / 7000)).pow(new Decimal(player.timePlayed / 9000)).pow(new Decimal(player.timePlayed / 5000)).pow(new Decimal(player.timePlayed / 30000)).pow(new Decimal(player.timePlayed / 150000)).pow(new Decimal(player.timePlayed / 750000)).pow(new Decimal(player.timePlayed / 2e6)).pow(new Decimal(player.timePlayed / 4e6)).pow(new Decimal(player.timePlayed / 6e6)).pow(new Decimal(player.timePlayed / 1e7)).pow(new Decimal(player.timePlayed / 1.6e7))))
            } else if (player.timePlayed < 40e6) {
                player.cf.ngu = notationChooser(new Decimal(player.timePlayed / (11.75 - Math.log10(player.timePlayed) - Math.log10(player.timePlayed / 60000))).pow(new Decimal(player.timePlayed / (0.3 / Math.max(Math.log10(player.timePlayed / 10000)+1, 1))).pow(new Decimal(player.timePlayed / (150 / Math.max(Math.log10(player.timePlayed / 225000)+1, 0.8)))).pow(new Decimal(player.timePlayed / (200 / Math.max(2*(Math.log10(player.timePlayed / 50000)+1)-1, 1)))).pow(new Decimal(player.timePlayed / 6000)).pow(new Decimal(player.timePlayed / 4000)).pow(new Decimal(player.timePlayed / 3000)).pow(new Decimal(player.timePlayed / 10000)).pow(new Decimal(player.timePlayed / 100000)).pow(new Decimal(player.timePlayed / 75000)).pow(new Decimal(player.timePlayed / 2e6)).pow(new Decimal(player.timePlayed / 4e6)).pow(new Decimal(player.timePlayed / 6e6)).pow(new Decimal(player.timePlayed / 1e7)).pow(new Decimal(player.timePlayed / 1.6e7)).pow(new Decimal(player.timePlayed / 2.5e7))))
            } else{
